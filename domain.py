...
class Domain:
    ...
    def __init__(self, num_superphotons):
        self.num_superphotons_init=num_superphotons
...
class OnezoneScattering(Domain):
    ...
    def __init__(self, **kwargs):
        ...
        # NOTE: only need two of the three variables of ne, tau and radius
        # optical depth of the sphere
        self.tau = kwargs["zone_tau"]
        # radius of scattering medium
        self.radius = kwargs["radius"]
        # number density of scattering medium
        self.ne = self.tau / self.radius / constants["sigma_thomson"]
        # superphotons list object. number will vastly increase as scatterings occur (why keep it?)
        self.superphotons = ([sp.Superphoton(self) for _ in range(self.num_superphotons)])
        self.num_scattered = 0
        # initial bias of superphotons
        try:
            self.bias_init = kwargs["bias_init"]
        except KeyError:
            self.bias_init = 1.0
        # geometry of problem
        ...
        self.spectra = np.zeros(shape=(self.n_scattering_bins,self.n_th_bins,len(self.freq_bins)))
    ...
    def init_superphotons(self):
        for i in range(self.num_superphotons):
            ...
            superphoton.nu = sampling.sample_1d_log(np.log(1e9),np.log(1e14))
            ...
            superphoton.k = k_spher*superphoton.energy
            superphoton.k_init = np.copy(k_spher)
            # stefanBoltzmann = 5.67e-5 / np.pi * (self.thetae * constants['me'] * constants['c']**2) / constants['kb']
            # superphoton.weight = self.bnu(superphoton.nu)/self.bnumax()[1] * self.num_superphotons / stefanBoltzmann
            superphoton.weight = self.bnu(superphoton.nu)/self.bnumax()[1] * 1e25
            
            # superphoton.weight=1e25
            superphoton.bias = superphoton.get_bias()
    ...
    def bnu(self,nu):
        ...
        if(exp_arg < 1e-6):
            ...
            return 2 * constants["h"] * nu**3 /(constants['c']**2) * 1/(np.exp(exp_arg)-1)

    def bnumax(self):
        """Planck function peak for weights"""
        numax = 2.8214391 / constants['h'] * self.thetabb * constants['me']* constants['c']**2
        return numax,self.bnu(numax)

    def sample_frequency(self):
        nu_peak, bnu_maxval = self.bnumax()
        nu_max = nu_peak*10
        nu_min = self.ln_nu0
    ...
    def record_superphoton(self, superphoton: sp.Superphoton):
        ...
        scattering_index = min(superphoton.times_scattered,self.n_scattering_bins-1)
        self.spectra[scattering_index,0,freq_bin_ind] += superphoton.weight * superphoton.energy
